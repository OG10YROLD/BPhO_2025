# Create an interactive model of the (surprising!) real image
# of an object in a concave spherical mirror.
...
def lens_project(x, y, R):
    #th = math.atan(y/(math.sqrt(R**2 - x**2)))
    th = math.atan(y/(math.sqrt(R**2 - y**2)))
    m = math.tan(2*th)
    X = -((m*math.sqrt(R**2 - y**2) - y))/((y/x) + m)
    Y = (y/x)*X
    return X, Y
...
def show():
    ...
    for row in range(H):
        for col in range(W):
            Xp[row, col], Yp[row, col] = lens_project(xw[(H-1) - row, (W-1) - col], yw[(H-1) - row, (W-1) - col], R)
    ...
    ax.plot(np.array([x, -math.sqrt(R**2-(y+h)**2)]), np.array([y+h, y+h]), color='r')
    ax.plot(np.array([-math.sqrt(R**2-(y+h)**2), lens_project(x, y+h, R)[0]]), np.array([y+h, lens_project(x, y+h, R)[1]]), color='r')
    ax.plot(np.array([x, lens_project(x, y+h, R)[0]]), np.array([y+h, lens_project(x, y+h, R)[1]]), color='r', ls='--')

    ax.add_patch(Arc((-math.sqrt(R**2-(y+h)**2), y+h), 0.2, 0.2, angle=-math.degrees(math.atan((y+h)/(math.sqrt(R**2 - (y+h)**2)))), theta1=0.0, theta2=math.degrees(math.atan((y+h)/(math.sqrt(R**2 - (y+h)**2)))), color='b'))
    ax.plot(np.array([-math.sqrt(R**2-(y+h)**2), 0]), np.array([y+h, 0]), color='b', ls='--')
    ...
    # Draw the lens
    ax.add_patch(Arc((0, 0), R*2, R*2, angle=0.0, theta1=90.0, theta2=270.0))
