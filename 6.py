# Create an interactive model of the real, inverted image
# of an object placed outside the focal range of an ideal thin lens.
...
def show():
    ...
    warped = cv2.warpPerspective(image, tform, (w_p, h_p), cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=(255, 255, 255))
    warped = np.flipud(warped) # The OpenCV coordinate system is opposite from matplotlib, so it needs to be flipped
    ax.imshow(warped, extent=(min_x, max_x, min_y, max_y)) # Draw the projected image
...
def on_press(event): # For moving the image with the keyboard
    ...
    match event.key:
        case "left":
            if (x - 0.15) > f: # Limit to outside focal range
                x -= 0.05
        case "right":
            x += 0.05
        case "up":
